...
export const PromptInputBox = React.forwardRef((props: PromptInputBoxProps, ref: React.Ref<HTMLDivElement>) => {
  const { 
    ...
    className,
    thinkingEnabled: externalThinkingEnabled,
    onThinkingChange,
  ...
  const [internalShowSearch, setInternalShowSearch] = React.useState(false);
  const [internalShowThink, setInternalShowThink] = React.useState(false);
  const [showCanvas, setShowCanvas] = React.useState(false);
  const uploadInputRef = React.useRef<HTMLInputElement>(null);
  const promptBoxRef = React.useRef<HTMLDivElement>(null);
  
  // Use external control if provided, otherwise use internal state
  const showThink = externalThinkingEnabled !== undefined ? externalThinkingEnabled : internalShowThink;
  const showSearch = externalWebSearchEnabled !== undefined ? externalWebSearchEnabled : internalShowSearch;
  
  const handleSetThink = (value: boolean) => {
    if (onThinkingChange) {
      onThinkingChange(value);
    } else {
      setInternalShowThink(value);
    }
  };
  
  const handleSetSearch = (value: boolean) => {
    if (onWebSearchChange) {
      onWebSearchChange(value);
    } else {
      setInternalShowSearch(value);
    }
  };

  const handleToggleChange = (value: string) => {
    if (value === "search") {
      handleSetSearch(!showSearch);
      handleSetThink(false);
    } else if (value === "think") {
      handleSetThink(!showThink);
